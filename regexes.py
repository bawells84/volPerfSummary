...
executing = re.compile('Executing\s(\w+)\((\d+)', re.MULTILINE)
...
vg_details = re.compile('Seq:\d+\s\/\sRAID\s(\d+)\s\/\s(\w+)\s\/\sTLP:(T|F)\s\/\sDLP:(T|F)\s\/\sSSM:\w\s\/\sActDrv:(\d+)\s\/\sInActDrv:(\d+)\s\/\sVolCnt:(\d+)\s\/\sSecure:(Enable|Disable)d?\nBlockSize:\d*\s\/\sPI\sCapable:(T|F)\s-\s(\d+)\s\/\sLabel:(\w+)', re.MULTILINE)
